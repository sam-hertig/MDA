###################################################################################################
#
# MULTI DOMAIN ASSEMBLER for Chimera v1.9 and up
...
def getUniprotID(pdbid, chain):
	...
	try:
		...
		#print 'Reading stored UniProt ID data...'
	except KeyError:
		try:
			#print 'Fetching UniProt ID data online...'
...
def elimAllGapColumns(structures):
	...
	# get rid of all gap columns (from Eric)
...
def processBlast(results, uid, path = '.', minscore = 50, includeNative = True, suppressdoubles = False, percentId = 0, winnow = '0', limit = 0, keepPDB = '', skipPDB = '', excludeSelected = False, deleteHidden = True, hideSubmodels = True, noConfirm = False):
	...
	try:
		parser = Parser("MDAquery", results, False)
	...
	# convert dots to bars due to new blast parser for version 2.2.29
	dot2Bars(structures)
	...
	# destroy old model panel groups
	destroyGroups()		

	# group submodels in model panel
	groupSubModels()		
...
def destroyGroups():
	...
	from ModelPanel.Group import Group
 	if _mp is None:
 		return		# Model Panel has not been created yet
	foundagroup=True
	while foundagroup:
		items = _mp.items[:]
		foundagroup=False
...
def writeFasta(structures, path, uid):
	...
	i = 0
	for s in structures[1:]:
		#fout.write( '>%d / %d  %s  %%ID %d \n' % (i, nrofhits, s.pdb, s.percentid) )
...
class Structure:
	...
	def getId(self):
		...
		# assume the set only contains one ID	
		self.uniprot = uniprotset.pop()
			...
			if len(chainIDs) > 1:
				...
				for id in chainIDs2:
					...
					for c in chains:
						...
						#print self.pdb, self.chain, self.ligands, reccenter, ligcenter
	...
	def seqStructMapping(self):
		...
		for chain in allchains:
			...
			# match the s.sequence residue numbers to pdb residue numbers 
 			try:
 				self.structseqobj = self.models[0].sequence(chain)
 			except KeyError:
 				...
 				continue
 			estlen, segments, gaps = estimateAssocParams(self.structseqobj)
 			try:
 				curmatchmap, errors = tryAssoc(self.seqobj, self.structseqobj, segments, gaps, estlen, maxErrors=errortolerance*estlen)
	 		except ValueError:
		...
		if nrofstructs > 1: # only prepare matchmaking if we have at least two structures
			for i in range(nrofstructs-1): # loop over structure-pairs in the box
				...
				if lent < 3 or lenc < 3 or lenc != lent:
					print 'Could not align %s to %s since not enough residues were found to call match.' % (curstruct.pdb, tempstruct.pdb)
					# if this is the case, coordN and coordC further below will have NaN instead of proper numbers
					# think about another way of placing the molecule, perhaps simply using overlaps and moves...
				else:	
					# hence, call match only if it will work properly
	...
	for b in Boxes:
		...
		while True:
			...
			if caN:
				...
				#print 'The N-terminal residue of %s was identified as "%s."' % (firststruct.pdb, str(resN))
				break
			i += 1
		...
		while True:
			...
			if caC:
				...
				#print 'The C-terminal residue of %s was identified as "%s."' % (laststruct.pdb, str(resC))
				break
			i += 1
		...
		for s in b.structures:
			if s.ishiddenunderneath:
				#print 'skipped', s.pdb, 'for measuring box size limits'
				continue
			#relevantatoms = [r.findAtom('CA') for r in s.models[0].residues if (r.findAtom('CA') and r.id.chainId == s.chain)]
		...
		if gaplength <= 0 and prevxmax != 0: # not sure whether we need the second condition...
 	...
 	#print 'Last gap measures', lastgaplength, 'residues.'
...
def openMAV(path, uid, structures):
	...
	templateModels = []
	for s in structures[1:]:
		i += 1 # seqs[0] is the query sequence, which we don't want to associated with any model
		mol = s.models[0]
		templateModels.append(mol)
		actualseq = mav.seqs[i]
		mav.associate([mol], seq = actualseq, force=True)
		app.update() # this ensures that the tk windows get redrawn so Windows users can see the progress while associating models to sequences
...
def main(results, uid, path, totseqlength, minscore = 50, includeNative = True, suppressdoubles = False, percentId = 0, group = False, hideSubmodels = True, hideComplex = False, hideAltChain = True, deleteHidden = True, coloring= 'mutations', winnow = '0', limit = '0', keepPDB = '', skipPDB = '', excludeSelected = False, showAlignment = True, noConfirm = False):
	...
	#from chimera.tkgui import app
	#app.update() # without this tk redraw update, MDA will crash on Windows for larger systems due to a mysterious bug
...
def mda(uniprotId = 'P02751', path = '~/Desktop/', minScore = 50, includeNative = False, suppressDoubles = False, percentId = 0, forceBlast = False, group = False, hideSubmodels = True, hideComplex = False, hideAltChain = True, deleteHidden = True, coloring= 'mutations', winnow = '0', limit = '0', keepPDB = '', skipPDB = '', excludeSelected = False, showAlignment = True, noConfirm = False):
	...
	# check whether the uniprot ID is actually an existing file
	if isfile(uniprotId):
		fasta = uniprotId
		ID = None
		...
		print 'Assembling protein with sequence specified in fasta file %s. Uniprot ID input DISREGARDED.' % fasta
	...
	def startBlast(querySeq = seq, winnow = winnow):		
		...
		from blastpdb.ParserBlastP import Blastp29Service
		program = None
		db = "pdbaanew"
		queryName = "MDAquery"
		evalue = "1000"
		matrix = "BLOSUM62"
		passes = None
		params = (program, db, queryName, querySeq, evalue, matrix, passes, winnow)
		print 'No preexisting BLAST database found, starting BLAST 2.2.29 using %s matrix...' % matrix

		def failCB():
			print "failed"
		def cancelCB():
			print "cancelled"
		bp29 = Blastp29Service(cb, params=params,
					failCB=failCB, cancelCB=cancelCB)
			
	if not forceBlast:	
		# open shelf
		blastshelf = shelve.open(filenameblast, 'c')
		# if entry exists, call main directly	
		try:
...
addCommand('mda', mdacommand, help = True) #someday!			


# Fn P02751  /  FilA P21333  /  FilB O75369  /  Talin-1 Q9Y490  /  gagpol P12497  /  VWF P04275
# trim5alpha Q0PF16, fibrillin (very long) P35555, tenascin P24821
# Twitchin Q23551 (huge y-stack...); Cadherin2 mus P15116; LFP5 O75197; Thrombospondin-1 P07996; Titin Q8WZ42 (BLAST fails); ; MMP?; Factor VIII P00451; 
